...
clf = joblib.load('text to audio/model3/models/gesture_classifier.pkl')
scaler = joblib.load('text to audio/model3/models/scaler.pkl')
label_encoder = joblib.load('text to audio/model3/models/label_encoder.pkl')
...
#     # Initialize landmarks for both hands (126 features = 63 for each hand)
#     landmarks = ['NaN'] * 126  # 126 features for both hands (if one hand is missing, fill with zeros)

#     # Draw hand landmarks and collect landmarks if hands are detected
#     if results.multi_hand_landmarks:
...
#             # If it's the first hand detected, store in the first 63 slots
#             if i == 0:
#                 landmarks[:63] = single_hand_landmarks
#             # If it's the second hand detected, store in the second 63 slots
#             elif i == 1:
#                 landmarks[63:] = single_hand_landmarks
...
#         # Convert landmarks to a numpy array for model prediction
#         landmarks = np.array(landmarks).reshape(1, -1)
